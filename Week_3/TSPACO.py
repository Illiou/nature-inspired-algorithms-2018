...
class TSPACO:
    def __init__(self, distance_matrix, initialization_value, evaporation_rate, intensification_value, alpha=1., beta=1.,
                 ant_number=50, n_best_to_intensify=1):
...
if __name__ == "__main__":
    distances_file = "../TSP_Problems/problem_02.tsp"
    problem = 2
    ...
    aco = TSPACO(distance_matrix, initial_pheromone_value, 0, intensification_value, alpha=1, beta=5, ant_number=20,n_best_to_intensify=5)
    ...
    plt.axhline(nicos_aco[problem],label = "ACO Benchmark")
    plt.axhline(nicos_hc[problem],label = "HC Benchmark")
    plt.legend()
    plt.show()
